...
#include <fstream> //ifstream、close()、seekg()、ofstream
#include <cmath>   //pow()
...
void split_text(int file_offset, int &file_eof); //拆文本
void en_decode_process();                        //一次编解码过程
...
int main()
{
    ...
    int code_size;   //记录编码大小
    int file_eof;    //记录读取的文本是否到达文件末尾 0无1有
    ...
    while (1)
    {
        split_text(file_offset, file_eof); //拆文本
        en_decode_process();               //一次编解码过程

        cout << endl;
        code_size = code_size + code.size(); //记录编码大小
        file_offset = file_offset + 25;      //文本偏移量增加

        if (file_eof == 1) //文本文件内容读完退出循环
        {
            break;
        }
    }
...
//拆文本
void split_text(int file_offset, int &file_eof)
{
    //打开文件
    // text.txt
    ...
    if (!in_file)
    {
        cerr << "The 'text.txt' doesn't exist" << endl; //标准错误输出
        exit(EXIT_FAILURE);                             //可用
    }

    // content.txt
    string file_name1; //文件名

    file_name1 = "./content.txt";

    ofstream out_file(file_name1, ios::out);
    //文件输出流类	使用流类的构造函数打开文件	只写，文件不存在新建，文件存在清楚  文本模式
    if (!out_file)
    {
        cerr << "The 'content.txt' doesn't exist" << endl; //标准错误输出
        exit(EXIT_FAILURE);                                //可用
    ...
    //文件定位
    in_file.seekg(file_offset, in_file.beg); //距离文件开头第X个字节

    //读取文件  从text.txt读25个字符写到content.txt
    char ch; //每字符
