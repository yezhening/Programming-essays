...
void loadavg_response(int sock_fd, struct sockaddr *clie_addr, int loadavg_flag);
...
void handle(int sock_fd, struct sockaddr *clie_addr)
{
    ...
    {
        //直接根据对象标识符处理    可扩展
        int loadavg_flag = 0;          //请求平均负载标志位 1一分钟5五分钟15十五分钟
        ...
        {
            if ((oid->value[0] == 0x29) && (oid->value[1] == 0x01)) //"1.1.1"请求一分钟平均负载
            {
                loadavg_flag = 1;
                loadavg_response(sock_fd, clie_addr, loadavg_flag);
            }
            else if ((oid->value[0] == 0x29) && (oid->value[1] == 0x02)) //"1.1.2"请求五分钟平均负载
            {
                loadavg_flag = 5;
                loadavg_response(sock_fd, clie_addr, loadavg_flag);
            }
            else if ((oid->value[0] == 0x29) && (oid->value[1] == 0x03)) //"1.1.3"请求十五分钟平均负载
            {
                loadavg_flag = 15;
                loadavg_response(sock_fd, clie_addr, loadavg_flag);
            }
...
void loadavg_response(int sock_fd, struct sockaddr *clie_addr, int loadavg_flag)
{
    ...
    //获取一个平均负载值
    char loadavg[10] = ""; //一个平均负载值
    int loadavg_i = 0;     //平均负载值下标

    if (loadavg_flag == 1)
    {
        for (int i = 0; i < strlen(result_buff); i++)
        {
            if (result_buff[i] == ' ')
            {
                break;
            ...
            loadavg[loadavg_i] = result_buff[i];
            loadavg_i++;
        }
    }
    else if (loadavg_flag == 5)
    {
        for (int i = 5; i < strlen(result_buff); i++) //投机取巧 0.00 0.00 0.00从5开始
        {
            if (result_buff[i] == ' ')
            {
                break;
    ...
    else if (loadavg_flag == 15)
    {
        for (int i = 10; i < strlen(result_buff); i++) //投机取巧 0.00 0.00 0.00从10开始
        {
            if (result_buff[i] == ' ')
            {
                break;
    ...
    strncpy(ia5_String.value, loadavg, strlen(loadavg));
