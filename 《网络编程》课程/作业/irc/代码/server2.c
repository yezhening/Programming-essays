...
int quit_connect_fd = -1; //记录需要关闭的连接套接字文件描述符
//在接收QUIT命令后，需要关闭接收QUIT命令的连接套接字文件描述符
...
void handle_req_data(struct Request_data *req_data);                                    //处理请求数据
...
void handle(int listen_fd)
{
    ...
    int close_flag = 0; //关闭连接套接字标志
    //时序逻辑：
    //客户端发送QUIT命令请求
    //服务端在Readv分支（第一次）：接收QUIT命令请求，处理请求数据时记录需要关闭的连接套接字
    //服务端在Readv分支：需要关闭的连接套接字：关闭连接套接字标志从0置1
    //服务端在Readv分支：所有已连接套接字提交Writev()请求
    //客户端接收quit内容响应，主动关闭连接
    //服务端在Writev分支：所有已连接套接字的客户端接收“has quit”内容响应，提交Readv()请求
    //服务端在Readv分支（第二次）：对需要关闭的连接套接字接收客户端关闭连接请求，关闭连接套接字标志为1，服务端关闭连接套接字
    //处理后退出分支，所有已连接套接字不提交Writev()请求
    //因为此时，需要关闭的连接套接字接收客户端关闭连接请求，不需要关闭的连接套接字处理请求数据为空(仍存在该情况)
    //优先处理客户端主动关闭连接情况，处理完后在第三次Readv分支，所有已连接套接字可提交Writev()请求

    //注意：关闭连接的四次挥手过程在close()系统调用由协议完成
    //当客户端主动关闭连接时，服务端连接套接字是读就绪状态，应处于提交Readv()请求阶段
    ...
    while (1)
    {
        ...
        case Accept:
            ...
            break;
        case Readv:
            //一个readv()请求已完成，处理请求数据，对所有已连接客户端提交writev()请求以准备发送数据给客户端
            handle_req_data(req_data); //处理请求数据

            if (close_flag == 1) //需要关闭连接套接字
            {
                if (req_data->connect_fd == quit_connect_fd) //是需要关闭连接套接字的文件描述符
                {
                    for (int i = 0; i < LISTEN_MAX_COUNT; i++)
                    {
                        if (client_data[i].connect_fd == quit_connect_fd) //需要关闭的连接套接字文件描述符
                        {
                            close(client_data[i].connect_fd); //服务端主动关闭连接套接字
            ...
            for (int i = 0; i < LISTEN_MAX_COUNT; i++) //对所有已连接客户端提交writev()请求以准备发送数据给客户端
            {
                if (client_data[i].connect_fd != -1) //已连接的
                {
                    if (client_data[i].connect_fd == quit_connect_fd) //需要关闭的连接套接字文件描述符
                    {
                        if (close_flag == 0)
                    ...
                    submit_writev(client_data[i].connect_fd, req_data); //提交Writev()请求
                }
            }
...
void submit_writev(int connect_fd, struct Request_data *req_data)
{
    ...
    memcpy(copy_req_data->io_vector[0].iov_base, req_data->io_vector[0].iov_base, copy_req_data->io_vector[0].iov_len); //记录发送数据
...
//处理请求数据
void handle_req_data(struct Request_data *req_data)
{
    ...
    else if (strstr(req_data->io_vector[0].iov_base, "QUIT:") != NULL) //是QUIT命令
    {
        for (int i = 0; i < LISTEN_MAX_COUNT; i++)
        {
            if (client_data[i].connect_fd == req_data->connect_fd) // 2.找到接收数据的连接套接字文件描述符  必找得到
            {
                quit_connect_fd = req_data->connect_fd; //记录需要关闭的连接套接字文件描述符
    ...
    memcpy(req_data->io_vector[0].iov_base, temp_data, req_data->io_vector[0].iov_len); //记录发送数据
