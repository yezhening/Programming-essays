...
#define LISTEN_MAX_COUNT 3 //所监听的最大连接数 两个客户端间通信
...
//客户端数据据
//有所监听的最大连接数个    记录与客户端建立连接的连接套接字文件描述符
int client_data[LISTEN_MAX_COUNT];
int client_count = 0; //记录与客户端建立连接的数量
...
void handle(int listen_fd)
{
    ...
    while (1)
    {
        ...
        case Accept:
            //连接建立后，记录客户端数据
            client_data[client_count] = cqe->res;
            client_count++;
        ...
        case Readv:
            ...
            {
                submit_writev(client_data[i], req_data);
            }
...
void submit_writev(int connect_fd, struct Request_data *req_data)
{
    ...
    copy_req_data->connect_fd = connect_fd;
    copy_req_data->io_vector[0].iov_base = malloc(BUFF_SIZE);
    copy_req_data->io_vector[0].iov_len = BUFF_SIZE;
    memcpy(copy_req_data->io_vector[0].iov_base, req_data->io_vector[0].iov_base, req_data->io_vector[0].iov_len);
