...
//————————————————————————————————————————
#define LOCKFILE "/var/run/sniffer.pid"						  //锁文件路径
#define LOCKMODE (S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)	  //打开锁文件模式
#define REQUESTFILE "/home/ricardo/桌面/Capture/request.pcap" //绝对路径	请求数据包文件路径
...
void readFile()
{
	...
	/* open capture device */
	handle = pcap_open_offline(REQUESTFILE, errbuf);
	if (handle == NULL)
	{
	...
	if (pcap_compile(handle, &fp, filter_exp, 0, PCAP_NETMASK_UNKNOWN) == -1) //不知网络掩码
	{
...
int main(int argc, char **argv)
{
	...
	int fd;
	if ((fd = open(LOCKFILE, O_RDWR | O_CREAT, LOCKMODE)) == -1)
	{
	...
	//保存文件
	pcap_dumper_t *dump;
	if ((dump = pcap_dump_open(handle, REQUESTFILE)) == NULL)
	{
		syslog(LOG_ERR, "Failed to open dump");
		exit(1);
	}
	pcap_loop(handle, num_packets, saveFile, (u_char *)dump);
