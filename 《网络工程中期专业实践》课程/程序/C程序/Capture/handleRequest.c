...
#define PAYLOAD_FIFE_PATH "/home/ricardo/桌面/RequestPackets/"   //保存载荷文件路径
...
void get_url(char *payload_content, char *url);
void get_user_agent(char *payload_content, char *user_agent);
void get_cookie(char *payload_content, char *cookie);
...
void save_file(int count, const u_char *payload, int size_payload)
{
    ...
    write(fd, payload_content, strlen(payload_content));
    close(fd);
...
/* 取Url    只取"get"方法的，可扩展取"Head"方法的 */
void get_url(char *payload_content, char *url)
{
    int i = 0;
    int j = 0;
    int get_flag = 0;
    int other_flag = 1; //其他请求方法的url返回host，无'/'

    // for (i = 0; i < strlen(payload_content); i++) //取url
    for (i = 0; i < strlen(payload_content); i++) //取url   截断过长的URL
    {
        if (payload_content[i] == 'G' && payload_content[i + 1] == 'E' && payload_content[i + 2] == 'T')
        {
            get_flag = 1;
            i = i + 5; //"GET /"
        ...
        if (payload_content[i] == ' ' && payload_content[i + 1] == 'H') //" H"TTP 
        {
            get_flag = 0;
        }
        ...
        if (get_flag == 1)
        {
            url[j] = payload_content[i];
            j++;
        }
    }

    char host[100] = "";             //因为字符串拼接，这里定义长一些
    get_host(payload_content, host); //取host
...
void get_user_agent(char *payload_content, char *user_agent)
{
    int i = 0;
    int j = 0;
    int user_agent_flag = 0;

    for (i = 0; i < strlen(payload_content); i++) //取user_agent
    {
        if (payload_content[i] == 'U' && payload_content[i + 1] == 's' && payload_content[i + 2] == 'e' && payload_content[i + 3] == 'r')
        {
            user_agent_flag = 1;
            i = i + 12; //"User-Agent: "
        }

        if (payload_content[i] == '.' && payload_content[i + 1] == '.') //".."
        {
            user_agent_flag = 0;
        }

        if (user_agent_flag == 1)
        {
            user_agent[j] = payload_content[i];
            j++;
        }
    }

    //存在"POST"请求方法"User-Agent"为空情况  数据库字段为"NOT NULL"
    if (strcmp(user_agent, "") == 0)
    {
        strcpy(user_agent, "none");
    }
}
...
void get_cookie(char *payload_content, char *cookie)
{
    ...
    for (i = 0; i < strlen(payload_content); i++) //取cookie
    {
        if (payload_content[i] == 'C' && payload_content[i + 1] == 'o' && payload_content[i + 2] == 'o' && payload_content[i + 3] == 'k')
        {
            cookie_flag = 1;
            i = i + 8; //"Cookie: "
        }
...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    char url[500] = "";
    get_url(payload_content, url);

    char user_agent[100] = "";
    get_user_agent(payload_content, user_agent);

    char cookie[500] = "";
    get_cookie(payload_content, cookie);
    ...
    // printf("%d,%s,%s,%d,%d,%s,%s,%s,%s,%d,%s\n", count - 1, source_ip, destination_ip, source_port, destination_port, host, url, user_agent, cookie, frame_size, file_path);

    // SQL语句
    char sql_statement[2000] = "insert into request(number,source_ip,destination_ip,source_port,destination_port,\
host,url,user_agent,cookie,frame_size,file_path) values(";
    ...
    strcat(sql_statement, ",'");
    strcat(sql_statement, host);
    strcat(sql_statement, "','");
    strcat(sql_statement, url);
    strcat(sql_statement, "','");
    strcat(sql_statement, user_agent);
    strcat(sql_statement, "','");
    strcat(sql_statement, cookie);
    strcat(sql_statement, "',");
