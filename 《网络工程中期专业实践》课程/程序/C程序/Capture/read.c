...
#define REQUESTFILE "/home/ricardo/桌面/Capture/a.pcap" //请求数据包文件路径  绝对路径
...
int main(int argc, char **argv)
{
    ...
    int num_packets = 10; /* number of packets to capture */ // pcap_loop

    /* open capture device */
    if ((handle = pcap_open_offline(REQUESTFILE, errbuf)) == NULL)
    {
        syslog(LOG_ERR, "Couldn't open file: %s", REQUESTFILE, errbuf);
        exit(EXIT_FAILURE);
    }
    ...
    {
        syslog(LOG_ERR, "%s is not an Ethernet", REQUESTFILE);
        exit(EXIT_FAILURE);
    }
